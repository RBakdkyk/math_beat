...
def _division(difficulty: str) -> dict:
    ...
    # medium: multi-digit ÷ single-digit, no remainder
    b = random.randint(2, 9)
...
_DENOM_PAIRS = {
    ...
    "medium": [(2, 4), (2, 6), (2, 8), (2, 10), (3, 6), (3, 9), (4, 8), (5, 10)],
    "hard":   [(2, 4), (2, 6), (3, 6), (3, 9), (4, 8), (5, 10)],
}
...
def _fraction_comparison(difficulty: str) -> dict:
    if difficulty == "easy":
        ...
        d1, d2 = random.choice([(2, 4), (2, 6), (3, 6), (3, 9), (4, 8), (5, 10)])
        a = random.randint(1, d1 - 1)
        b = random.randint(1, d2 - 1)
        while Frac(a, d1) == Frac(b, d2):
            b = random.randint(1, d2 - 1)
    else:
        d1 = random.choice([3, 4, 5, 6, 7, 8])
        a = random.randint(1, d1 - 1)
        d2 = random.choice([d for d in [3, 4, 5, 6, 7, 8] if d != d1])
        b = random.randint(1, d2 - 1)
        while Frac(a, d1) == Frac(b, d2):
            b = random.randint(1, d2 - 1)
...
def _fraction_addition(difficulty: str) -> dict:
    ...
    result_str = _frac_str(Frac(a, d1) + Frac(b, d2))
    desc = random.choice([
        "חשב/י:",
        "מה הסכום?",
        "חבר/י שברים:",
    ])
    return _q(desc, f"{a}/{d1} + {b}/{d2} =", result_str, "numeric", "fraction-addition",
...
def _fraction_subtraction(difficulty: str) -> dict:
    ...
    for _ in range(20):
        a = random.randint(1, d1)
        b = random.randint(1, d2)
        if Frac(a, d1) >= Frac(b, d2):
            break
    else:
        a, b = d1, 1  # fallback: d1/d1 - 1/d2
    result_str = _frac_str(Frac(a, d1) - Frac(b, d2))
    desc = random.choice([
        "חשב/י:",
        "מה ההפרש?",
    ])
    return _q(desc, f"{a}/{d1} - {b}/{d2} =", result_str, "numeric", "fraction-subtraction",
...
def _exponents(difficulty: str) -> dict:
    ...
    return _q(desc, f"{base}^{exp} =", base ** exp, "numeric", "exponents", f"exp:{base}^{exp}")
...
def _multiplication_by_tens(difficulty: str) -> dict:
    ...
    if difficulty == "easy":
        ...
        b = random.choice([100, 1000, 200, 300, 500, 400])
    else:
        a = random.randint(2, 99)
        b = random.choice([10, 100, 20, 30, 40, 50, 200, 300])
    desc = random.choice([
        "חשב/י:",
        "מה המכפלה?",
        "כפול/י:",
...
def _arithmetic_sequences(difficulty: str) -> dict:
    if difficulty == "easy":
        ...
        step = random.choice([5, 10, 15, 20, 25, 50, 100])
        length = 6
    else:
